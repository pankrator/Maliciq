...
//		addToImpact(GameWindow.hero);
	...
	protected void moving() {
		...
		if (!GameWindow.nodeMap.getNode(gridX, gridY).isFree()) {
			...
			// FIXME When impact with GameWindow.hero only the first bullet is
			// destroyed
			if (canImpact(GameWindow.nodeMap.getNode(gridX, gridY)
					.getRandomObj())) {

				GameWindow.destroyInstanceAtPosition(GameWindow.nodeMap
						.getNode(gridX, gridY).getRandomObj());
			}

			if (!GameWindow.nodeMap.getNode(gridX, gridY).getObj()
					.contains(shooter)) {
				GameWindow.createEffect(new CircleEffect(gridX, gridY,
						Color.GREEN, 200));
	...
	public boolean canImpact(GameObject go) {
		return (canImpactList.contains(go.getClass()) && !forbiddenList
				.contains(go));
	}
